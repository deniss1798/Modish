...
TAG='modish-backend:release7-20260916'
NETWORK='modish-release7-test'
PG='modish_release7_test_pg'
...
def main():
    ...
    old=inspect('modish_backend');assert old['Config']['Image']=='modish-backend:release6-20260915'
    ...
    with (R/'smoke.log').open('w') as log:
        run('docker','run','--rm','--name','modish_release7_smoke','--network',NETWORK,'--env-file',str(R/'test-api.env'),
            '-v',str(R/'smoke_release7.py')+':/smoke.py:ro','--entrypoint','python',TAG,'/smoke.py',stdout=log,stderr=subprocess.STDOUT)
    assert all(inspect(n)['Id']==cid for n,cid in state.items())
