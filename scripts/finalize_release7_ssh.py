...
assert inspect('modish_backend')['Config']['Image']=='modish-backend:release7-20260916'
test=inspect('modish_release7_test_pg')
assert test['Name']=='/modish_release7_test_pg'
assert set(test['NetworkSettings']['Networks'])=={'modish-release7-test'}
...
subprocess.run(['docker','rm','--force','--volumes','modish_release7_test_pg'],check=True,stdout=subprocess.DEVNULL)
network=json.loads(subprocess.check_output(['docker','network','inspect','modish-release7-test']))[0]
assert not network['Containers']
subprocess.run(['docker','network','rm','modish-release7-test'],check=True,stdout=subprocess.DEVNULL)
