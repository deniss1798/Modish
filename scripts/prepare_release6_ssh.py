...
REMOTE='/opt/modish-releases/20260915-release6'
...
with sock,sock.makefile('r',encoding='utf-8') as incoming:
 ...
 try:
  ...
  sftp=c.open_sftp();sftp.mkdir(REMOTE,mode=0o700)
  uploads=[(Path(os.environ['TEMP'])/'modish-release6.tar','source.tar'),
    (ROOT/'scripts/prepare_release6_remote.py','prepare.py'),(ROOT/'scripts/smoke_release6.py','smoke_release6.py')]
  for local,name in uploads:sftp.put(str(local),REMOTE+'/'+name);sftp.chmod(REMOTE+'/'+name,0o600)
  sftp.close()
  command="python3 - <<'PY'\nimport subprocess\nfrom pathlib import Path\nr=Path('/opt/modish-releases/20260915-release6')\nwith (r/'prepare.log').open('w') as log:\n p=subprocess.Popen(['python3','-u',str(r/'prepare.py')],stdin=subprocess.DEVNULL,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)\nprint('Preparation started:',p.pid)\nPY"
