...
TAG='modish-backend:release6-20260915'
NETWORK='modish-release6-test'
PG='modish_release6_test_pg'
...
def main():
    ...
    old=inspect('modish_backend');assert old['Config']['Image']=='modish-backend:styling-fixes-20260913'
    ...
    with (R/'migration-test.log').open('w') as log:
    ...
    with (R/'smoke.log').open('w') as log:
        run('docker','run','--rm','--name','modish_release6_smoke','--network',NETWORK,'--env-file',str(R/'test-api.env'),
            '-v',str(R/'smoke_release6.py')+':/smoke.py:ro','--entrypoint','python',TAG,'/smoke.py',stdout=log,stderr=subprocess.STDOUT)
    assert all(inspect(n)['Id']==cid for n,cid in state.items())
