...
assert inspect('modish_backend')['Config']['Image']=='modish-backend:styling-fixes-20260913'
test=inspect('modish_styling_test_pg')
assert test['Name']=='/modish_styling_test_pg'
assert set(test['NetworkSettings']['Networks'])=={'modish-styling-test'}
...
subprocess.run(['docker','rm','--force','--volumes','modish_styling_test_pg'],check=True,stdout=subprocess.DEVNULL)
network=json.loads(subprocess.check_output(['docker','network','inspect','modish-styling-test']))[0]
assert not network['Containers']
subprocess.run(['docker','network','rm','modish-styling-test'],check=True,stdout=subprocess.DEVNULL)
